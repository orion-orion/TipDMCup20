...
LastEditTime: 2021-02-21 01:02:45
'''
...
#  基分类器定义
models = {}
models.update({'dt': DecisionTreeClassifier(max_depth=8)})
# models.update({'rf':
#     RandomForestClassifier(n_estimators=500, warm_start=True, max_features='sqrt', max_depth=6,
#     min_samples_split=3, min_samples_leaf=2, n_jobs=-1, verbose=0)
# })
models.update({'knn': KNeighborsClassifier(n_neighbors=3)})
# models.update({'et':
#     ExtraTreesClassifier(n_estimators=500, n_jobs=-1, max_depth=8, min_samples_leaf=2, verbose=0)
# })
...
param_grids.update({
    'dt':
    {'min_samples_split': [2, 4], 'max_depth': [20]}
})
# param_grids.update({
#     'rf':
#     {'n_estimators': [500], 'min_samples_split': [2, 3], 'max_depth': [20]}
# })
param_grids.update({
    'knn':
...
# param_grids.update({
#     'et':
#    {'n_estimators': [500], 'min_samples_split': [3, 4], 'max_depth': [20]}
# })

#  完成超参数网格搜索后的模型
model_grids = {}
for name,  param in param_grids.items():
    model_grids[name] = model_selection.GridSearchCV(
        models[name], param, n_jobs=-1, cv=10, verbose=1)
